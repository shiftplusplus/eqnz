...
```{r, fig.cap="Proportion of earthquakes at night: U.S.A. n=278458"}
...
```{r, fig.cap="Proportion of night earthquakes by magnitude, U.S.A. n=278458"}
old_par=par()
...
```{r, fig.cap="Over- and under- supply of earthquakes by angle of the sun (10 degree steps). U.S.A. n=276458"}
norm_ci$border = 2
...
second <- first[2:length(first)]
second <- second - seconds(1)
mid_url <- paste("starttime=", format(first, "%Y-%m-%d"), "%20", 
                 format(first, "%H:%M:%S"),
                 "&endtime=", format(second, "%Y-%m-%d"), "%20", 
                 format(second, "%H:%M:%S"), sep="")
secondlast <- "&maxlatitude=50&minlatitude=24.6&maxlongitude=-65&minlongitude=-125&"
full_url <- paste("https://earthquake.usgs.gov/fdsnws/event/1/query.csv?", 
                  mid_url,secondlast,
                  "minmagnitude=0&eventtype=earthquake&orderby=time", sep="")
usgs <- lapply(full_url[1:(length(full_url)-2)],read.csv, stringsAsFactors=FALSE)
equsa <- bind_rows(usgs)
rm(usgs)
equsa$time_UTC <- as.POSIXct(gsub("\\..+","",as.character(equsa$time)), 
                             format="%Y-%m-%dT%H:%M:%S", tz="UTC")
eq_national <- equsa %>% 
  filter(depth > 0 & mag >= 0 & 
           time_UTC >= 
...
## (10 degree steps). U.S.A. n=276458"----
norm_ci$border = 2
