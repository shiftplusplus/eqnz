...
For the same period as the New Zealand Data (from September 2011 to September 2016), in earthquakes data from the Australia there are `r eq_total` events of depth greater than 0 and magnitude greater than 0.
...
Of the `r eq_total` in the data, `r eq_night` occurred at night, a proportion of `r round(eq_night / eq_total,4)`.  A seven sigma confidence interval for the proportion of earthquakes occurring at night is `r round(bt$conf.int[1],4)` to `r round(bt$conf.int[2],4)`, so I can be confident this is a non-random difference. This very high rate is narrowly smaller than the rate observed in the Philippines.

```{r, fig.cap="Proportion of earthquakes at night: Philippines. n=8054"}
...
```{r, fig.cap="Proportion of night earthquakes by magnitude, Philippines. n=8054"}
old_par=par()
