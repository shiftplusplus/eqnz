...
For the same period as the New Zealand Data (from September 2011 to September 2016), in earthquakes data from the Philippines there are `r eq_total` events of depth greater than 0 and magnitude greater than 0.
...
Of the `r eq_total` in the data, `r eq_night` occurred at night, a proportion of `r round(eq_night / eq_total,4)`.  A seven sigma confidence interval for the proportion of earthquakes occurring at night is `r round(bt$conf.int[1],4)` to `r round(bt$conf.int[2],4)`, so I can be confident this is a non-random difference. 
...
```{r, fig.cap="Over- and under- supply of earthquakes by angle of the sun (10 degree steps). , Philippines. n=8054"}
norm_ci$border = 2
