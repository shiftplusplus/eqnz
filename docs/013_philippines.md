...
obs <- unlist(strsplit(gsub("([0-9]+) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)","~\\1 \\2",h), "~" ))
...
obs <- gsub("[[:space:]]+"," ", obs)
obs <- gsub("~ ","~",obs)
obsdf <- data.frame(obs) %>% separate(obs, into=c("plpdate", "everything_else"), sep="~", extra="merge") %>%
  separate(everything_else, into=c("latitude", "longitude", "depth", "magnitude", "location"), sep=" ", extra="merge") %>%
  mutate(time_UTC = as.POSIXct(plpdate, format="%d %B %Y - %I:%M %p", tz="UTC") - hours(8),
...
eq_national <- eq_philippines %>% filter(depth > 0 & magnitude >= 0 &
...
eq_philippines$longitude[eq_philippines$longitude > 9000] <- eq_philippines$longitude[eq_philippines$longitude > 9000] /100
rm(eq_philippines)
...
## ---- fig.cap="Over- and under- supply of earthquakes by angle of the sun (10 degree steps). , Philippines. n=8054"----
norm_ci$border = 2
