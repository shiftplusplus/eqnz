...
```{r fig.cap="Distribution of Magnitude (steps of .1)"}
...
```{r fig.cap="Distribution of Depth (steps of 10 km)"}
...
```{r fig.cap="Depth and Magnitude of earthquakes"}
...
```{r fig.cap="Number of earthquakes by magnitude, split by hour of day (UTC)"}
...
```{r fig.cap="Number of earthquakes by Magnitude and Classification"}
eqnz %>% filter(magnitude > 0, depth > 0) %>%
...
Latitudes range from `r min(eqnz$latitude)` to `r max(eqnz$latitude)`, while longitudes range from `r min(eqnz$longitude)` to `r max(eqnz$longitude)` (thought it might be better to say longitudes from `r min(eqnz$longitude[eqnz$longitude > 0])` to `r max(eqnz$longitude[eqnz$longitude < 0])`).
...
```{r fig.cap="Variation in angle of sun by hour of day"}
